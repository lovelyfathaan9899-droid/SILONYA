...
- [ ] Test coverage per TESTING_STRATEGY.md — the "Unit" layer (§3) now exists: Vitest wired in (`packages/config/vitest/base.ts`, `pnpm test` in CI), with 67 tests covering `packages/utils`'s pricing/shipping/tax/discount math and `packages/api`'s discount-code validation, order/email mapper, and rate limiter — the money-critical logic PROJECT_RULES.md §1 flags as non-negotiable. Integration tests against a real Postgres DB (§4 — needed for `services/inventory.ts`'s raw-SQL reservation logic), Playwright E2E (§5), automated accessibility (§6), Lighthouse CI (§7), and visual regression (§8) are still unbuilt
