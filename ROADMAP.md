...
- [ ] Test coverage per TESTING_STRATEGY.md — the "Unit" layer (§3) exists: Vitest wired in (`packages/config/vitest/base.ts`, `pnpm test` in CI), with 67 tests covering `packages/utils`'s pricing/shipping/tax/discount math and `packages/api`'s discount-code validation, order/email mapper, and rate limiter. The "Integration" layer (§4) has started: `pnpm test:integration` runs real-Postgres tests (a dedicated Neon database locally, a `postgres:16` CI service container in place of the doc's originally-specified Neon-branch-per-run, for lack of Neon API access — see TESTING_STRATEGY.md §4's implementation-status note) covering `checkout.createIntent` end-to-end and the oversell-prevention concurrency case (PRODUCT_SYSTEM.md §4.3) specifically — the rest of the mutating procedures §4 calls for, webhook-handler tests, Playwright E2E (§5), automated accessibility (§6), Lighthouse CI (§7), and visual regression (§8) are still unbuilt
