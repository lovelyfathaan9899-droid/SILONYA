...
├── cart        (get, addItem, updateItem, removeItem, applyDiscount)
├── checkout    (createIntent, confirm, getOrderConfirmation)
...
- Mutations that touch money or inventory (`checkout.confirm`, `admin.orders.updateStatus`) are wrapped in a single DB transaction (DATABASE_ARCHITECTURE.md §5).
...
| `cart.addItem`                | mutation | `{ variantId: string, quantity: number }`                             | updated `Cart`                                    | public (guest via session cookie)           |
| `checkout.createIntent`       | mutation | `{ cartId, shippingAddress, billingAddress }`                         | `{ clientSecret, orderId }`                       | public                                      |
| `checkout.confirm`            | mutation | `{ orderId, paymentIntentId }`                                        | `Order`                                           | public (ownership verified via order token) |
...
| -------------------------------------------------- | ------ | ----------------------------------------- | ------------------------------- |
| `/api/v1/webhooks/stripe`                          | POST   | Payment/refund/dispute events from Stripe | Stripe signature verification   |
