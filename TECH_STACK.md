...
| Testing          | **Vitest**                                                                                               | Native ESM/TS support matches every package's `"type": "module"`; faster than Jest for this stack. Unit-tests the pure/mockable business logic PROJECT_RULES.md §1 requires (`packages/utils`, `packages/api`) via `pnpm test` — config shared via `packages/config/vitest/base.ts`, same pattern as the shared ESLint/TypeScript configs. A separate integration layer (`packages/api/vitest.integration.config.ts`, `pnpm test:integration`, TESTING_STRATEGY.md §4) runs real-Postgres tests against a dedicated test database — see `.env.example`'s `TEST_DATABASE_URL`/`TEST_DIRECT_URL`. Component/E2E coverage for `apps/web`/`apps/admin` is not yet set up (Phase 11 gap) |
